			...
			try {
				...
				while(items.hasNext()) {
					...
					if(item.isFormField()) {
						...
						File path = new File(getServletContext().getRealPath("/") + "tmp/covers/" + album.getInterpreter());
						...
						uploadedFile = new File(path + "/" + album.getName() + "." + item.getContentType().split("/")[1]);
						...
						try {
							...
							album.setCoverPicture("storage/covers/" + album.getInterpreter() + "/" + album.getName() + "." + item.getContentType().split("/")[1]);
							request.getSession().setAttribute("cover", "tmp/covers/" + album.getInterpreter() + "/" + album.getName() + "." + item.getContentType().split("/")[1]);
